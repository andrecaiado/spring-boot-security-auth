...
- `UserDetailsService`: As we are using a username and password authentication mechanism, we need to provide a `UserDetailsService` to retrieve the user details from the database.
...
In the [SecurityConfiguration.java](src%2Fmain%2Fjava%2Fcom%2Fexample%2Fspringbootsecurityauth%2Fconfig%2FSecurityConfiguration.java) class, we configure the `AuthenticationManager` and the `PasswordEncoder`.
